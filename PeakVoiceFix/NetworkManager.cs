...
namespace PeakVoiceFix
{
    public class CacheEntry
    {
        ...
        public byte RemoteState; // [新增] 存储远端玩家的 ClientState (转为byte)
    }
    ...
    public static class NetworkManager
    {
        ...
        private const byte TYPE_STATE = 2; // [新增] 状态同步包
        ...
        public static string GetPlayerName(int actorNumber)
        {
            ...
            if (PlayerCache.ContainsKey(actorNumber))
            {
                ...
                if (!string.IsNullOrEmpty(cached) && cached != "Unknown") return cached;
            }

            Photon.Realtime.Player player = null;
            if (PhotonNetwork.CurrentRoom != null) player = PhotonNetwork.CurrentRoom.GetPlayer(actorNumber);

            if (player != null && !string.IsNullOrEmpty(player.NickName)) resultName = player.NickName;
        ...
        public static void UpdatePlayerCache(int actorNumber, string name, string ip = null)
        {
            ...
            if (!string.IsNullOrEmpty(name) && name != "Unknown") PlayerCache[actorNumber].PlayerName = name;
        ...
        public static void SendStateSync(ClientState state)
        {
            ...
            // 内容: [Type(2), State(byte)]
            object[] content = new object[] { TYPE_STATE, stateByte };
            RaiseEventOptions opts = new RaiseEventOptions { Receivers = ReceiverGroup.Others };
            // 使用不可靠传输以减少网络拥堵，因为状态是连续变化的
        ...
        public static void OnEvent(EventData photonEvent)
        {
            if (photonEvent.Code == 186)
            {
                ...
                if (photonEvent.CustomData is object[] data && data.Length >= 2)
                {
                    ...
                    else if (type == TYPE_STATE) // [新增] 处理状态包
                    {
                        byte state = 0;
                        ...
                        if (PlayerCache.ContainsKey(senderActor))
                        {
                            ...
                            PlayerCache[senderActor].LastSeenTime = Time.unscaledTime; // 续命
                        }
                        else
                        {
                            var entry = new CacheEntry { PlayerName = senderName, LastSeenTime = Time.unscaledTime, RemoteState = state };
